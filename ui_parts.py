...
def _mobile_safe_time_input_row(
    ...
    hour_field = _cupertino_input(
        value=hour_value,
        on_change=lambda _value: None,
        placeholder=f"{_normalize_display_text(label)} (時)",
        keyboard_type=ft.KeyboardType.NUMBER,
        width=124,
        height=52,
        text_align=ft.TextAlign.CENTER,
    )
    minute_field = _cupertino_input(
        value=minute_value,
        on_change=lambda _value: None,
        placeholder="分",
        keyboard_type=ft.KeyboardType.NUMBER,
    ...
    def sync_from_fields(_e: ft.ControlEvent) -> None:
        composed = _compose_time_value(hour_field.value or "0", minute_field.value or "0")
        normalized_hour, normalized_minute = _split_time_value(composed)
        hour_field.value = normalized_hour
        minute_field.value = normalized_minute
    ...
    hour_field.on_blur = sync_from_fields
    minute_field.on_blur = sync_from_fields
    hour_field.on_submit = sync_from_fields
    minute_field.on_submit = sync_from_fields
    ...
    return ft.Column(
        controls=[
            ...
            ft.Text("時: 0〜23 / 分: 0〜55（5分刻み）", size=FONT_SIZE_XS, color=COLOR_GRAY_TEXT),
        ],
        spacing=6,
    )
